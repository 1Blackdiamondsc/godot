...
def configure(env):
    ...
    angle_build_cmd = f'msbuild.exe {angle_root}/winrt/10/src/angle.sln /nologo /v:m /m:{jobs} /p:Configuration=Release /p:Platform='
    ...
    if str(os.getenv("Platform")).lower() == "arm":
        ...
        if env["bits"] == "32":
            ...
            env.Append(LIBPATH=[f'{os.environ["VCINSTALLDIR"]}lib/store/amd64'])
    ...
    env.Append(
        CCFLAGS=[
            "/AI",
            f'{vc_base_path}\\vcpackages',
            "/AI",
            f'{os.environ["WINDOWSSDKDIR"]}\\References\\CommonConfiguration\\Neutral',
        ]
    )
